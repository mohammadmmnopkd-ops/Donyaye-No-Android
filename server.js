...
function payoutPlayer(p){
  ...
  if(ticks>0){p.money+=ticks*PAYOUT_DOLLARS;p.lastPayoutAt += ticks*PAYOUT_MS;return ticks}
  return 0;
}
...
app.get('/api/game/rules',(req,res)=>res.json({ok:true,payoutMinutes:10,payoutDollars:3000,payoutPerHour:18000,recruitmentHours:24}));
